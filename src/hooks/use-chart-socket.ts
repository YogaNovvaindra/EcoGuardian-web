...
import { esp } from "@prisma/client"; // Import model Sensor dari Prisma
...
export const useChartSocket = ({ addKey, updateKey, queryKey }: SensorSocketProps) => {
  ...
  useEffect(() => {
    if (socket) {
      const handleAddKey = (sensorData: esp) => {
        // Tampilkan notifikasi saat ada data baru dari WebSocket
        showNotification("Data Sensor Baru", { body: `Temperatur: ${sensorData.latitude}°C` });
      };
  ...
  useEffect(() => {
    ...
    socket.on(updateKey, (sensorData: esp) => {
      queryClient.setQueryData([queryKey], (oldData: any) => {
        ...
        const newData = oldData.pages.map((page: any) => {
          return {
            ...page,
            sensorData: page.sensorData.map((item: esp) => {
    ...
    socket.on(addKey, (sensorData: esp) => {
      queryClient.setQueryData([queryKey], (oldData: any) => {
        if (!oldData || !oldData.pages || oldData.pages.length === 0) {
          return {
            pages: [
